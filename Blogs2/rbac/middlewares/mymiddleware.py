...
class Auth(MiddlewareMixin):
    ...
    def process_request(self,request):
        # 登录认证白名单
        white_list = [reverse('rbac:login'),]
        # 权限认证白名单
        permission_white_list = [reverse('rbac:index'), '/admin/*','/favicon.ico']#/admin/login/?next=/admin/  # 加上'',用于通过127.0.0.1:8000
        ...
        bread_crumb = [
            {'url': reverse('rbac:index'), 'title': '首页'},
