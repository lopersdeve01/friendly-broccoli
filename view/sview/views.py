...
class NewFilterBackend1(BaseFilterBackend):
    def filter_queryset(self, request, queryset, view):
        # val = request.query_params.get('pk')
        val = request.GET.get('pk')
        print(val)
        return queryset.filter(pk=val)
...
class NewArticleView1(ListAPIView,CreateAPIView):
    ...
    #                 condition['category']=category
    #             queryset=models.Article.objects.filter(**condition).order_by('-data')
    queryset = models.Article.objects.all().order_by('-data')
    ...
    #         serializer_class = PageArticleSerializer
    #     else:
    #         serializer_class = [ArticleListSerializer,ArticleDetailSerializer]
    #         filter_backends = [NewFilterBackend1]
    #         def perform_create(self, serializer):
    #
    #             article_object = ser.save(author_id=1)
    #             ser_detail.save(article=article_object)
    #
    #             perform_create(self,serilizer_class)
    #         filter_backends = [NewFilterBackend1]
...
class NewArticleView2(RetrieveAPIView):
...
class NewCommentView1(ListAPIView,CreateAPIView):
    # super().__init__(request, *args, **kwargs_)
...
# class NewCommentView2(RetrieveAPIView):
#     queryset = models.Article.objects.all()
