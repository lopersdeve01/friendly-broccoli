...
def menu_list(request):
    ...
    if mid:
        permission_query = models.Permission.objects.filter(Q(menus_id=mid) | Q(parent__menus_id=mid))
    else:
        permission_query = models.Permission.objects.all()

    all_permission = permission_query.values('id', 'url', 'title', 'url_name', 'menus_id', 'parent_id', 'menus__name')
...
def multi_permissions(request):
    ...
    # 数据库中的所有权限的别名
    permissions_name_set = set([i.title for i in permissions])
